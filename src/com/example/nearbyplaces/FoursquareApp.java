	...
	public ArrayList<FsqVenue> getNearby(double latitude, double longitude)
		...
		try {
			...
			url = new URL(API_URL + "/venues/search?ll=" + ll + "&oauth_token="
					+ mSession.getAccessToken() + "&v=" + v + "&limit=50");
			...
			if (length > 0) {
				...
				for (int j = 0; j < ilength; j++) {
					...
					JSONObject caterogyObj= (JSONObject) categoriesArray.get(0);
					 img_URL=caterogyObj.getJSONObject("icon").getString("prefix")+"bg_44"+caterogyObj.getJSONObject("icon").getString("suffix");
...
//						+ item.getString("suffix");
				img_URL = item.getString("prefix") +"50x50" + item.getString("suffix");
